...
use crate::hash::{FontHasher, FontHasherBuilder};
...
pub struct TableKern {
    pub horizontal_mappings: HashMap<u32, i16, FontHasherBuilder>,
}
...
impl TableKern {
    ...
    fn read_format0(stream: &mut Stream) -> Option<HashMap<u32, i16, FontHasherBuilder>> {
        let pairs = stream.read_u16()?;
        stream.skip(6); // searchRange: u16, entrySelector: u16, rangeShift: u16
        let mut mappings = HashMap::with_hasher(FontHasherBuilder);
        for _ in 0..pairs {
    ...
    fn read_format3(stream: &mut Stream) -> Option<HashMap<u32, i16, FontHasherBuilder>> {
